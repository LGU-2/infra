    ...
    subgraph aws["AWS ap-northeast-2"]
        ...
        subgraph vpc["VPC 10.0.0.0/16"]
            ...
            subgraph pri["프라이빗 서브넷 10.0.11.0/24 AZ-a, 10.0.12.0/24 AZ-c"]
                RDS1[("RDS MySQL 8.4<br/>db.t4g.micro<br/>primary AZ-a<br/>PITR 보존 7일")]
                RDS2[("standby AZ-c<br/>읽기 불가, 페일오버 전용")]
...
│     ├── app-1        SG-app  : SG-alb 와 SG-mon 에서 오는 8080
│     ├── monitoring   SG-mon  : 인바운드 없음. 관리자 접근은 SSM 포트 포워딩
...
| 모니터링 -> 앱 | 보안 그룹 참조로 8080. 익스포터 포트는 9장 |
...
SG-app    인바운드 8080     <- SG-alb, SG-mon
...
**`SG-mon` 에 인바운드가 없는 것이 맞다.** 스크랩은 모니터링이 앱으로 나가는 연결이고, 응답은 보안 그룹이 상태를 추적해 자동으로 돌아온다. Grafana 화면은 SSM 포트 포워딩으로 본다. `SG-batch` 와 `SG-lt` 도 ALB 에 붙지 않고 외부에서 들어올 일이 없어 비어 있다.
