...
sequenceDiagram
    ...
    alt 객체가 있고 서명 조건과 일치한다
        A->>A: UPDATE upload_status='CONFIRMED', byte_size=실측값
        A-->>C: 201 Created
...
올라온 것이 조건과 같은지는 여기서만 확인된다. `byte_size` 를 실측값으로 덮어쓰는 것도 이 시점이다.
...
UPDATE product_image
   SET upload_status = 'CONFIRMED', byte_size = ?
...
그래서 정리 배치가 함께 필요하다. 1시간이 지난 `PENDING` 행을 골라 **S3 객체를 먼저 지우고 그 다음 행을 지운다.**
순서가 반대면 key 를 잃어버려 객체를 영영 못 찾는다. `DeleteObject` 는 없는 객체에도 성공하므로 첫 줄 같은 경우도 같은 경로로 정리된다.

```
0 ~ 1시간   클라이언트 재시도 / 조회 확정이 되살린다
1시간 이후   배치가 객체와 행을 지운다. 조회는 더 이상 S3 를 묻지 않는다
```
