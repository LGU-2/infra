...
이 저장소에서 CI 가 보는 것은 **문서와 `items.yml` 의 일치 하나뿐**이고, 그것만 병합을 막는다.
...
**빌드가 없어 커버리지와 정적 분석 게이트도 없다.** 그 둘과 LLM 판정(G-PR)은 `fm-backend` 가 맡는다.
판정 대상 코드가 이 저장소에 없기 때문이다. Terraform 이 생기면 그때 다시 정한다.
